     ...
     if j < 2 then
        ...
        Memo1.Lines.Add(x + y);
         ...
         for i := 1 to j do begin
              for k := Length(y) downto 1 do begin
                  res := res + y[k];
              end;
              { Rekursivt anrop. }
...
begin
    ...
    res := '';
    instring := Edit1.Text;
    for i := Length(instring) downto 1 do begin
        res := res + instring[i];
    end;
        Memo1.Lines.Add(res);
    end;
...
begin
    ...
    MessageDlg('Fil är skapad, med sökväg och namn: ' + mapp +'\Permutation of ' + word + ' .txt', mtInformation, [mbOk] , 0);
...
begin
  ...
  ClickedOK := InputQuery('DAGSSLÄNDAN', 'Antal tecken att flytta till slutet:', SAntalTecken);
